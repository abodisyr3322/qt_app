...
if is_admin():
    ...
    class Main(QMainWindow): #QWidget
        ...
        def creating(self,fromW):
            ...
            img = img.resize((220,125),Image.ANTIALIAS)
            ...
            if fromW!="Local":
                ...
                if self.programeNameShow !="":
                    ...
                    if listImportant[2]!="":
                        ...
                        self.programeNameE.setText(listImportant[2])
        ...
        def summaryReports(self):
            ...
            self.TableSummary.setGeometry(0,0,720,500)
        ...
        def resizeSummary(self,ev):
            ...
            self.heightChanged = self.windowsummary.height() - self.prihei
            ...
            self.prihei = self.windowsummary.height()
        ...
        def updateAReport(self):
            ...
            if self.ablePrograme:
                namePrograme = self.programeNameE.toPlainText()
            if self.ableGoals:
                Goals = self.programeGoalsE.toPlainText()
            if self.ableDescription:
                description = self.programeDescriptionE.toPlainText()
            if self.ableCreator:
                executer = self.programeCreatorE.toPlainText()
            if self.ableDate:
                executeDate = self.programeWhenDateE.toPlainText()
            if self.ableBenefits:
                benefits = self.programeBenefitsE.toPlainText()
            if self.ableCount:
                countBenefits = self.CountBenefitsE.toPlainText()
            if self.countPic != 0:
        ...
        def writeWord(self,fromWhere="Word"):
            ...
            if len(FileNameSave[0])>0:
                ...
                if self.ablePrograme:
                    ...
                    hdr_Cells[0].text = self.programeNameE.toPlainText()
                ...
                if self.ableGoals:
                    ...
                    hdr_Cells[0].text = self.programeGoalsE.toPlainText()
                ...
                if self.ableDescription:
                    ...
                    hdr_Cells[0].text = self.programeDescriptionE.toPlainText()
                ...
                if self.ableCreator:
                    ...
                    hdr_Cells[0].text = self.programeCreatorE.toPlainText()
                ...
                if self.ableDate:
                    ...
                    hdr_Cells[0].text = self.programeWhenDateE.toPlainText()
                ...
                if self.ableBenefits:
                    ...
                    hdr_Cells[0].text = self.programeBenefitsE.toPlainText()
                ...
                if self.ableCount:
                    ...
                    hdr_Cells[0].text = self.CountBenefitsE.toPlainText()
                ...
                if self.countPic != 0:
                    ...
                    for i in range(len(self.pictersPaths)):
                        if self.pictersPaths[i] !="":
                                ...
                                resize = imgB.resize((500,500),Image.ANTIALIAS)
        ...
        def completeExportAllSummaryReports(self,idFun):
            ...
            if countBenefits!="":
                ...
                hdr_Cells = programeCountBenefitsTable.rows[0].cells
                hdr_Cells[1].text = "عدد المستفيدين"
                hdr_Cells[0].text = countBenefits
                programeCountBenefitsTable.autofit = False
            ...
            convert(f"tempPdf/{name2}",f"tempPdf/{str(idFun)}.pdf")
...
1- الآمان
3- تعديل طريق الحفظ للوورد و pdf بحيث أني اكتب اسم التقرير واحدد مكان الحفظ -> done
2- تكبير صفحة التقارير المحفوظة -> done
7- عند كتابة التقرير في الحقول يكون عربي بشكل تلقائي
4- تغيير شعار وزارة التعليم -> done
5- حذف اسم مدرستنا -> done
6- صفحة ملخص التقارير الأيقونات تكون من اليمين لليسار . -> done
8- اضافة زر حفظ -> done
9- إمكانية طباعة كامل التقرير اللي موضح فيها اسم البرنامج وتاريخ التنفيذ و .... -> done
10- اضافة زر للوحة التحكم في صفحة إنشاء تقرير -> done
11-عرض وحذف -> لملخص التقارير اضافة الى تغيير ملخص التقارير الى التقارير المحفوظة
